  ...
  plugins: [
    ...
    },
    {
      ...
      fn: function (source) {
       ...
       // console.log(list)
        fs.writeFileSync(path.join(__dirname, `../src/lhy.json`), JSON.stringify(list, null, 2))
        for (let dir in dirs) {
          dirs[dir].sort((a, b) => {
...
component: () => import('./demos/NotFoundComponent.vue').then(m => m.default)
        ...
        fs.writeFileSync(path.join(__dirname, `../src/lhy.json`), JSON.stringify(source, null, 2))
       // console.log(source)
        return source
      }
    },
